...
- **Verifica su NAS**: scansione automatica (`handleCheckNasFiles`) al primo accesso al tab Archivio (`nasFiles === null`), nessun bottone manuale. Icona `Database` (lucide-react) sempre presente su ogni riga: grigia = non ancora verificato, verde (`text-emerald-400`) = trovato sul NAS nel nuovo schema `{TEMA}/{NATURA}/{codice}-titolo.ext`, ambra = ambiguo (più file candidati nel vecchio archivio ADAM OLD, nessuna azione automatica), rossa = non trovato — tooltip nativo (`title=`) con la spiegazione. La scansione migra automaticamente in un unico passaggio i video trovati solo nel vecchio schema ADAM OLD (`POST /migrate-legacy-file` sul NAS → `renameSync`, mai sovrascrive una destinazione esistente), poi mostra un riepilogo (spostati/saltati con motivo). Per ri-verificare basta ricaricare la pagina. Bottone rapido **"SteadyTube"** (icona `Database` + testo) accanto a "Modifica" su ogni riga, visibile solo quando l'icona non è già verde: richiama `/api/save-to-nas` con i dati correnti della riga e ri-scansiona subito dopo il successo. Stessa etichetta "SteadyTube" sulla checkbox "Salva anche su SteadyTube" nel tab "In attesa". Vedi memoria `project_nas_archive_verification` per lo schema esatto del vecchio archivio e per il fix del toast di esito (`nasApproveMsg`, che resta visibile per tutta la durata del salvataggio, non solo a operazione conclusa).
